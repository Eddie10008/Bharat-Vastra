// Australian market configuration for server-side
...
const calculateShippingCost = (orderTotal) => {
  return orderTotal >= AUSTRALIA_CONFIG.shipping.freeThreshold ? 0 : AUSTRALIA_CONFIG.shipping.standardCost;
};

// Calculate tax amount
const calculateTax = (subtotal) => {
  return subtotal * AUSTRALIA_CONFIG.tax.rate;
};

// Validate Australian postcode
const validatePostcode = (postcode) => {
  return AUSTRALIA_CONFIG.address.postcodeValidation.test(postcode);
};

// Get state abbreviation
const getStateAbbreviation = (stateName) => {
  return AUSTRALIA_CONFIG.address.stateAbbreviations[stateName] || stateName;
};
