...
export const numerologyProfiles = {
  1: {
    name: "The Pioneer",
...
export const getNumerologyProfile = (lifePathNumber) => {
  return numerologyProfiles[lifePathNumber] || null;
};

export const calculateDiscount = (lifePathNumber, basePrice) => {
...
export const getPersonalizedRecommendations = (lifePathNumber) => {
  const profile = getNumerologyProfile(lifePathNumber);
  if (!profile) return [];
  
  return profile.productRecommendations;
};

export const getPersonalizedColors = (lifePathNumber) => {
  const profile = getNumerologyProfile(lifePathNumber);
...
export const getPersonalizedGemstones = (lifePathNumber) => {
  const profile = getNumerologyProfile(lifePathNumber);
  if (!profile) return [];
  
  return profile.gemstones;
};

export const getPersonalizedThemes = (lifePathNumber) => {
  const profile = getNumerologyProfile(lifePathNumber);
  if (!profile) return [];
...
// Generate personalized CSS variables for the user's theme
export const generatePersonalizedCSS = (lifePathNumber) => {
  const colors = getPersonalizedColors(lifePathNumber);
  if (!colors) return '';
  
  return `
    :root {
      --numerology-primary: ${colors.primary};
      --numerology-secondary: ${colors.secondary};
      --numerology-accent: ${colors.accent};
      --numerology-primary-light: ${colors.primary}20;
      --numerology-secondary-light: ${colors.secondary}20;
      --numerology-accent-light: ${colors.accent}20;
    }
  `;
};
